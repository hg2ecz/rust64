    ...
    fn calculate_filter(&mut self) {
        ...
        match self.filter_type {
            ...
            FilterType::Bandpass => {
                ...
                self.d2 = -1.0;
                self.iir_att =
                    0.25 * (1.0 + self.g1 + self.g2) * (1.0 + (f32::consts::PI * arg).cos()) / (f32::consts::PI * arg).sin();
            }
            FilterType::Notch => {
                ...
                match self.voices[i].state {
                    ...
                    VoiceState::Decay => {
                        if (self.voices[i].level <= self.voices[i].sustain_level) || (self.voices[i].level > 0xFFFFFF) {
                            ...
                            self.voices[i].level = self.voices[i]
                                .level
                                .wrapping_sub(self.voices[i].decay_sub >> EGDR_SHIFT[(self.voices[i].level >> 16) as usize]);
                            if (self.voices[i].level <= self.voices[i].sustain_level) || (self.voices[i].level > 0xFFFFFF) {
                                self.voices[i].level = self.voices[i].sustain_level;
                            }
                        }
                    ...
                    VoiceState::Release => {
                        self.voices[i].level = self.voices[i]
                            .level
                            .wrapping_sub(self.voices[i].release_sub >> EGDR_SHIFT[(self.voices[i].level >> 16) as usize]);
                        if self.voices[i].level > 0xFFFFFF {
