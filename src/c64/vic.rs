...
impl VIC {
        ...
        match self.curr_cycle {
            ...
            1 => {
                if self.raster_cnt == (NUM_RASTERLINES - 1) {
                    ...
                    self.draw_this_line =
                        (self.raster_cnt >= FIRST_DISP_LINE) && (self.raster_cnt <= LAST_DISP_LINE) && !self.frame_skipped;
                }
    ...
    fn draw_std(&mut self, color: &[u8]) {
        ...
        self.window_buffer[screen_pos + 7] = utils::fetch_c64_color_rgba(color[(data & 1) as usize]);
        data >>= 1;
        self.window_buffer[screen_pos + 6] = utils::fetch_c64_color_rgba(color[(data & 1) as usize]);
        data >>= 1;
        self.window_buffer[screen_pos + 5] = utils::fetch_c64_color_rgba(color[(data & 1) as usize]);
        data >>= 1;
        self.window_buffer[screen_pos + 4] = utils::fetch_c64_color_rgba(color[(data & 1) as usize]);
        data >>= 1;
        self.window_buffer[screen_pos + 3] = utils::fetch_c64_color_rgba(color[(data & 1) as usize]);
        data >>= 1;
        self.window_buffer[screen_pos + 2] = utils::fetch_c64_color_rgba(color[(data & 1) as usize]);
        data >>= 1;
        self.window_buffer[screen_pos + 1] = utils::fetch_c64_color_rgba(color[(data & 1) as usize]);
        data >>= 1;
    ...
    fn draw_multi(&mut self, color: &[u8]) {
        ...
        self.fg_mask_buffer[self.fg_mask_offset] |= ((self.gfx_data & 0xAA) | (self.gfx_data & 0xAA) >> 1) >> self.x_scroll;
        self.fg_mask_buffer[self.fg_mask_offset + 1] |=
            ((((self.gfx_data & 0xAA) | (self.gfx_data & 0xAA) >> 1) as u16) << (8 - self.x_scroll)) as u8;

        let mut data = self.gfx_data;
        self.window_buffer[screen_pos + 7] = utils::fetch_c64_color_rgba(color[(data & 3) as usize]);
        data >>= 2;
        self.window_buffer[screen_pos + 6] = self.window_buffer[screen_pos + 7];
        self.window_buffer[screen_pos + 5] = utils::fetch_c64_color_rgba(color[(data & 3) as usize]);
        data >>= 2;
        self.window_buffer[screen_pos + 4] = self.window_buffer[screen_pos + 5];
        self.window_buffer[screen_pos + 3] = utils::fetch_c64_color_rgba(color[(data & 3) as usize]);
        data >>= 2;
        ...
        for snum in 0..8 {
            ...
            if ((self.sprite_draw & sbit) != 0) && (self.mx[snum] < (c64::SCREEN_WIDTH as u16) - 32) {
                ...
                if (mxe & sbit) != 0 {
                    ...
                    let mut sdata_l: u32;
                    let mut sdata_r: u32;
                    let mut fg_mask_r: u32 =
                        ((self.fg_mask_buffer[fmbp + 4] as u32) << 24) | ((self.fg_mask_buffer[fmbp + 5] as u32) << 16);
