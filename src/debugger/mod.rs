...
impl Debugger {
    ...
    fn draw_border(&mut self) {
        for x in 0..80 {
            self.font
                .draw_char(&mut self.window_buffer, DEBUG_W, 8 * x as usize, 0, 64, 0x0B);
            self.font
                .draw_char(&mut self.window_buffer, DEBUG_W, 8 * x as usize, 8 * 27, 64, 0x0B);
        }

        for y in 1..27 {
            self.font
                .draw_char(&mut self.window_buffer, DEBUG_W, 8 * 40, 8 * y as usize, 66, 0x0B);
        }
    ...
    fn draw_vic_window_txt(&mut self) {
        ...
        self.font
            .draw_text_rgb(&mut self.vic_buffer, RASTER_DEBUG_W, 11, 5, "Border on", BORDER_COLOR);
        self.font
            .draw_text_rgb(&mut self.vic_buffer, RASTER_DEBUG_W, 11, 6, "Bad line", BADLINE_COLOR);
        self.font
            .draw_text_rgb(&mut self.vic_buffer, RASTER_DEBUG_W, 11, 7, "Reg. change", VIC_WRITE_COLOR);
        self.font
            .draw_text_rgb(&mut self.vic_buffer, RASTER_DEBUG_W, 11, 8, "Raster IRQ", RASTER_COLOR);
    }
